...
nxfn = @(x,y,z) x./sqrt(x.^2/a^4 + y.^2/b^4 + z.^2/b^4)/a^2;
nyfn = @(x,y,z) y./sqrt(x.^2/a^4 + y.^2/b^4 + z.^2/b^4)/b^2;
nzfn = @(x,y,z) z./sqrt(x.^2/a^4 + y.^2/b^4 + z.^2/b^4)/b^2;

                             
exactfnu = @(x,y,z) 1/beta * ( alpha*exactfnv(x,y,z) + dxfnv(x,y,z).*nxfn(x,y,z) + dyfnv(x,y,z).*nyfn(x,y,z) + dzfnv(x,y,z).*nzfn(x,y,z) );
phi = @(x,y,z) x.^2/a^2 + y.^2/b^2 + z.^2/b^2 - 1;                             
...
cpf = @(x,y,z) cpEllipsoidInterior(x,y,z,[a b]);
cpfS = @(x,y,z) cpEllipsoid(x,y,z,[a b]);
...
%     EV_bdy = interp3_matrix(a_x1d{i},a_y1d{i},a_z1d{i},a_xcp{i}(a_bdyg{i}),a_ycp{i}(a_bdyg{i}),a_zcp{i}(a_bdyg{i}),p,a_band{i});
%     cpxv_double_bdy = 2*cpxv_bar_bdy - a_xcp{i}(a_bdyg{i});
%     cpyv_double_bdy = 2*cpyv_bar_bdy - a_ycp{i}(a_bdyg{i});
%     cpzv_double_bdy = 2*cpzv_bar_bdy  - a_zcp{i}(a_bdyg{i});
%     EdoubleV_bdy = interp3_matrix(a_x1d{i},a_y1d{i},a_z1d{i},cpxv_double_bdy,cpyv_double_bdy,cpzv_double_bdy,p,a_band{i});
    Iv = speye(size(Lv{i}));
    ng = nnz(a_bdyg{i});
    D = spdiags(a_distg{i}(a_bdyg{i}),0,ng,ng);
    Av{i} = -Lv{i} + Iv;
    A_bdy = alpha*D*(Iv(a_bdyg{i},:) + EbarV_bdy)/2 + (-EbarV_bdy + Iv(a_bdyg{i},:))/2;
    %A_bdy = alpha*D*(Iv(a_bdyg{i},:) + 3*EbarV_bdy - EdoubleV_bdy) / 3 + (EdoubleV_bdy/6 - EbarV_bdy + EV_bdy/2 + Iv(a_bdyg{i},:)/3);
    Av{i}(a_bdyg{i},:) =  A_bdy;
...
[xp, yp, zp] = paramEllipsoid(256,[a b]);
...
for start = 1:1:n_level-1
    ...
    [umg{start}, vmg{start}, err_inf_u(start,:), err_inf_v(start,:)] = ...
...
colorbar;
set(gca,'Fontsize',12)
print(h3,'-dpng','-r450','couple-ellipsoid-u.png');


h4 = figure(4);
Eplotv = interp3_matrix( a_x1d{i}, a_y1d{i}, a_z1d{i}, xp1, yp1, zp1, p, a_band{i} );
vplot = Eplotv*vmg{i};
vplot = reshape(vplot, size(xp));
surf(xp, yp, zp, vplot);
xlabel('\fontsize{15}x'); ylabel('\fontsize{15}y'); zlabel('\fontsize{15}z');
axis equal; shading interp;
colorbar;
set(gca,'Fontsize',12)
print(h4,'-dpng','-r450','couple-ellipsoid-v.png');