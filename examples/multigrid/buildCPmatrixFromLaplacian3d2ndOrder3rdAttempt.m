function [E,DiagwLinverse] = buildCPmatrixFromLaplacian3d2ndOrder3rdAttempt(x, y, z, xi, yi, zi, p, band, use_ndgrid)
...
% second [\Delta u](cp) by what we know from the orginal PDE.
  ...
  % hard code some of the extra weights, attempting to make 2nd-order, but
  % preserve positivity.
  
  % interpolation of u_xxx
  weights(:,18:4:30) = weights(:,18:4:30) - (dx-2*hx)/3/dx^3 .* (dy-hy)/dy .* (dz-hz)/dz * [-1,3,-3,1];
  weights(:,19:4:31) = weights(:,19:4:31) - (dx-2*hx)/3/dx^3 .* hy/dy      .* (dz-hz)/dz * [-1,3,-3,1];
  weights(:,34:4:46) = weights(:,34:4:46) - (dx-2*hx)/3/dx^3 .* (dy-hy)/dy .* hz/dz      * [-1,3,-3,1];
  weights(:,35:4:47) = weights(:,35:4:47) - (dx-2*hx)/3/dx^3 .* hy/dy      .* hz/dz      * [-1,3,-3,1];
  % interpolation of u_yyy 
  weights(:,21:1:24) = weights(:,21:1:24) - (dy-2*hy)/3/dx^3 .* (dx-hx)/dx .* (dz-hz)/dz * [-1,3,-3,1];
  weights(:,25:1:28) = weights(:,25:1:28) - (dy-2*hy)/3/dx^3 .* hx/dx      .* (dz-hz)/dz * [-1,3,-3,1];
  weights(:,37:1:40) = weights(:,37:1:40) - (dy-2*hy)/3/dx^3 .* (dx-hx)/dx .*  hz/dz     * [-1,3,-3,1];
  weights(:,41:1:44) = weights(:,41:1:44) - (dy-2*hy)/3/dx^3 .* hx/dx      .*  hz/dz     * [-1,3,-3,1];
  % interpolation of u_zzz 
  weights(:,6:16:54)  = weights(:,6:16:54)  - (dz-2*hz)/3/dx^3 .* (dx-hx)/dx .* (dy-hy)/dy * [-1,3,-3,1];
  weights(:,10:16:58) = weights(:,10:16:58) - (dz-2*hz)/3/dx^3 .* hx/dx      .* (dy-hy)/dy * [-1,3,-3,1];
  weights(:,7:16:55)  = weights(:,7:16:55)  - (dz-2*hz)/3/dx^3 .* (dx-hx)/dx .* hy/dy * [-1,3,-3,1];
  weights(:,11:16:59) = weights(:,11:16:59) - (dz-2*hz)/3/dx^3 .* hx/dx      .* hy/dy * [-1,3,-3,1];
  %tic
