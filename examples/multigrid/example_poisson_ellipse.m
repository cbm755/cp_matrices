...
% TODO: fix this. 
% uexactfn = @(t) sin(t);
% rhsfn = @(t,r) -sin(t)./(a^2*sin(t).^2 + b^2*cos(t).^2) - (a^2-b^2)*sin(t).*cos(t).^2./(a^2*sin(t).^2 + b^2*cos(t).^2).^2;

n = 10;
m = 1;
uexactfn = @(t) sin(n*t) + sin(m*t);
...
%     [umg err_inf(start,:) res(start,:)] = ...
%         gmg_test(Mc1, Lc1, Ec, V, F1, TMf2c, TMc2f, a_band, R, n1, n2, start, w, uexact, Eplot, MAX);
...
elseif n_level == 6
    ...
    legend('N=160','N=80','N=40','N=20','N=10')
elseif n_level == 4
    semilogy(n,res(1,:),'.-',n,res(2,:),'r.-',n,res(3,:),'c^-');
...
ylabel('\fontsize{15} |residual|_{\infty}')%title(['sin(\theta) with p=', num2str(p), ',  res = E*(f-L*v)'])
...
if n_level == 8
    semilogy(xx,rep_err_inf_matlab(1,:),'b',xx,rep_err_inf_matlab(2,:),'r',xx,rep_err_inf_matlab(3,:),'c', ...
         xx,rep_err_inf_matlab(4,:),'k',xx,rep_err_inf_matlab(5,:),'g', xx,rep_err_inf_matlab(6,:),'m-d', ...
...
title('\fontsize{15} error on an ellipse ')
