...
bandinit_c = find(abs(dist_c) <= bw*dx_c);
distg_c = dist_c(bandinit_c);

M = round(log(dx_c/dx)/log(2));
[bandinit, xinit, yinit, cpxinit, cpyinit, distg, dx, x1d, y1d] = ...
refine_grid(M, cpf, dx_c, x1d_c, y1d_c, bw, bandinit_c, distg_c);
...
disp('constructing operators and two bands')
[L, E, R, iband, oband, iband2, oband2, innerInOuter] = ...
    ops_and_bands2d_test(x1d,y1d, xinit,yinit, cpxinit,cpyinit, bandinit, p, order);
disp('two bands and corresponding operators have been successfully built')

% Want to test what will happen if we only do extension to points in 'oband\iband'.
% It is obviously not wise to first compute rows of E, and then let them
% be 1 and 0. Trying to seperate the process of constructing bands and 
% matrices, want to build bands independently instead of by finding the 
% nonzero elements of matrices E & L.
disp('letting E only do extension to points in oband\iband')
tic;
E(innerInOuter,:) = speye(length(iband));  %  R(:,innerInOuter) = I.
toc;
disp('done')
...
%xp = xp(:); yp = yp(:);
[xp,yp,thp] = param(600);
Eplot = interp2_matrix_test(x1d, y1d, xp, yp, p);
Eplot = Eplot(:,iband2);
