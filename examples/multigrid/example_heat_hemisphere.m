...
disp('dealing with boundary condition')
% here F and pt are just for passing the parameter right; hoping to fix this.
F = []; pt = [1 0 0];
[Mc, Lc, Ec, F] = app_bnd3d(Mc, Lc, Ec, F, a_xcp, a_ycp, a_zcp, a_bdyg, pt, 'dirichlet');
disp('done')
...
for kt = 1:numtimesteps
    ...
    F{1} = u;
    unew = gmg(Mc, Lc, Ec, V, F, TMf2c, TMc2f, a_band, a_bdyg, n1, n2, 1, w, uexactfn(t, th_plot, phi_plot), Eplot, MAX);
