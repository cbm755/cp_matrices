...
for i = 1:1:n_level
    ...
    E = E(:,a_band{i});
    Mc{i} = E*Lc{i} - 2*dim/ddx^2*(speye(size(Ec{i}))-Ec{i});
...
%uexactfn1 = @(th) exp( cos(th) );
%uexactfn = @(th) uexactfn1(th) - uexactfn1(0);
 
%rhsfn = @(th,r) ( 121*exp( cos(11*th) ).*( sin(11*th).^2 - cos(11*th) ) );
%uexactfn1 = @(th) exp( cos(11*th) );
%uexactfn = @(th) uexactfn1(th) - uexactfn1(0);
...
% n_mode = 50;
% rhsfn = @(th,r) rhsfn_mix_mode(th,n_mode);
% uexactfn = @(th) uexactfn_mix_mode(th,n_mode);

 m = 11;
 rhsfn = @(th,r) (-sin(th) - m^2*sin(m*th));
 uexactfn = @(th) sin(th) + sin(m*th);

%  rhsfn = @(th,r) -sin(th) - 4*sin(2*th) - 9*sin(3*th) - 16*sin(4*th) - 25*sin(5*th);
...
for j = 1:1:length(a_xg{i})
            if abs(a_xg{i}(j)-pt1(1)) < tol && abs(a_yg{i}(j)-pt1(2)) < tol
...
% [Mc, Lc, Ec, F] = app_bnd(Mc, Lc, Ec, F, a_xcp, a_ycp, a_bdyg, pt, 'neumann');
...
elseif n_level == 8
    semilogy(n,err_matlab(1,:),'.-',n,err_matlab(2,:),'r.-',n,err_matlab(3,:),'c^-', ...
            n,err_matlab(4,:),'k-s',n,err_matlab(5,:),'g+--',n,err_matlab(6,:),'m-d', ...
            n,err_matlab(7,:),'b*--');
    legend('N=640', 'N=320','N=160','N=80','N=40','N=20','N=10')
end
...
n = 1:MAX;
if n_level == 9
    semilogy(n,res2(1,:),'x-',n,res2(2,:),'r.-',n,res2(3,:),'c^-', ...
...
n = 1:MAX;
if n_level == 9
    semilogy(n,res1(1,:),'x-',n,res1(2,:),'r.-',n,res1(3,:),'c^-', ...
...
elseif n_level == 8
    semilogy(n,err_inf(1,:),'.-',n,err_inf(2,:),'r*-',n,err_inf(3,:),'c^-', ...
        n,err_inf(4,:),'k-s',n,err_inf(5,:),'g+--',n,err_inf(6,:),'m-d',n,err_inf(7,:),'b.--');
    legend('N=640', 'N=320','N=160','N=80','N=40','N=20','N=10');
end
...
set(gca,'Fontsize',20)
xlabel('\fontsize{20} number of vcyles')
ylabel('\fontsize{20} |error|_{\infty}')
title('\fontsize{20} error comparing to the exact solution ')
