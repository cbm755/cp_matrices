function [u] = helper_vcycle_t(Mc_t, DMc_t, Lc_t, Ec, V, F, TMf2c, TMc2f, BAND, BOUNDARY, n1, n2, start, w)
...
for i = start:1:n_level-1
    ...
    for j = 1:1:n1
    %V{i} = gauss_seidel(Mc_t{i}, V{i}, F{i});
    V{i} = jacobi_t(Mc_t{i}, DMc_t{i}, V{i}, F{i});
    %V{i} = jacobi(Lc{i}, V{i}, F{i});
    %V{i} = Ec{i}*V{i};
    end
    
    %res = F{i} - Mc_t{i}*(Ec{i}*V{i});
    res = F{i} - Mc_t{i}*V{i};
...
if isa(Mc_t, 'cell')
    V{n_level} = Mc_t{n_level} \ F{n_level};
    %[V{n_level} flag] = gmres(Mc_t{n_level}, F{n_level}, [], 1e-8);
else
    %V{n_level} = Mc \ F{n_level};
    [V{n_level} flag] = gmres(Mc_t, F{n_level}, [], 1e-8);
end
