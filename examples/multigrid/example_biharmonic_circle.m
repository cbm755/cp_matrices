...
uexactfn = @(t,th) exp(-t)*cos(th); %+ exp(-81*t)*cos(3*th);
...
V = cell(n_level,1);
F = cell(n_level,1);
bi_Mc = cell(n_level,1);
bi_Lc = cell(n_level,1);
DMc = cell(n_level,1);
for i = 1:1:n_level
    %A_Mc{i} = speye(size(Mc{i})) - 0.5*dt*Mc{i};
    %A_Lc{i} = speye(size(Lc{i})) - 0.5*dt*Lc{i}; 
    %M = Ec{i}*Lc{i};
    %bi_Mc{i} = speye(size(Ec{i})) + dt*( M*M + 2*dim/dx^2*(speye(size(Ec{i})) - Ec{i}) );
    %bi_Mc{i} = speye(size(Ec{i})) + dt*( M*M );
    bi_Mc{i} = speye(size(Ec{i})) + dt*( Mc{i}*Mc{i} );
    DMc{i} = diag(bi_Mc{i});
end 
...
for kt = 1:numtimesteps
	...
	%f = u + 0.5*dt*Lc{1}*u;
    %unew = A_Mc{1} \ u;
    %[unew flag] = gmres(bi_Mc{1}, u, [], 1e-10);
    res = Eplot{1}*(u - bi_Mc{1}*unew);
    %r = norm(res,inf)
    %u = Ec{1}*unew;
    u = unew;
...
for kt = 1:numtimesteps
	...
	%F{1} = u + 0.5*dt*Lc{1}*u;
    F{1} = u;
    unew = gmg_M(bi_Mc, bi_Lc, Ec, V, F, TMf2c, TMc2f, a_band, a_bdyg, n1, n2, 1, w, uexactfn(t,thetas), Eplot, MAX);
    %unew = helper_vcycle_M(bi_Mc, bi_Lc, Ec, V, F, TMf2c, TMc2f, a_band, a_bdyg, n1, n2, 1, w);
    toc;
    res = Eplot{1}*(u - bi_Mc{1}*unew);
    %r = norm(res,inf)
