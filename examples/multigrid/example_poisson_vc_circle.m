...
for i = 1:1:n_level
    ...
    [Sxb, Sxf, Syb, Syf] = sum_nb_2d_matrices(x,y,a_band{i});
    coef_xb = Sxb*beta; 
    coef_xf = Sxf*beta;
    coef_yb = Syb*beta;
    coef_yf = Syf*beta;
    ...
    I = speye(size(Lc{i}));
	%Mc{i} = lapsharp(Lc{i}, Ec{i}, 2*dim/dx1^2);
    Mc{i} = Ec{i}*Lc{i} - 2*dim/a_dx{i}^2*(I-Ec{i});
end
...
title('error comparing to the exact solution ')
