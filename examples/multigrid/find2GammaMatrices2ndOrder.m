function [GAMMA1, GAMMA2] = find2GammaMatrices2ndOrder(x, y, z, xi, yi, zi, p)
...
%         gammas(ind,ijk) = 0;
  ...
  [xwL,diag_x] = LaplacianWeightsNonUniform1D_vec(dx,hx);
  [ywL,diag_y] = LaplacianWeightsNonUniform1D_vec(dy,hy);
  [zwL,diag_z] = LaplacianWeightsNonUniform1D_vec(dz,hz);
    
  % weights from diagonal entries of Laplacian.
  diagwL = diag_x + diag_y + diag_z;
  
  for k = 1:N
      for i = 1:N
          for j = 1:N
              ijk = sub2ind([N,N,N], j, i, k);
              weightsL(:,ijk) = weightsL(:,ijk) + xwL(:,i) .* yw3(:,j) .* zw3(:,k);
              weightsL(:,ijk) = weightsL(:,ijk) + ywL(:,j) .* xw3(:,i) .* zw3(:,k);
              weightsL(:,ijk) = weightsL(:,ijk) + zwL(:,k) .* xw3(:,i) .* yw3(:,j);
          end
      end
  end
