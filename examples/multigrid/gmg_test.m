function [v error_circ_inf_mg res] = gmg_test(M, L, E, V, F, TMf2c, TMc2f, BAND, BDYG, n1, n2, start, w, uexact, Eplot, MAX)
...
while cnt < MAX
   ...
   v = v1;
   V{start} = v;
   res(cnt+1) = norm(Eplot{start}*(F{start} - L{start}*v),inf);
   r = res(cnt+1)
