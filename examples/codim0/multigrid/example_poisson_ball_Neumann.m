%% test geometric multigrid method to solve -\Delta u + u  = f in a ball, Neumann B.C.s
shift = 1;
% uexactfn = @(x,y,z) exp(sin(x.*y.*z));
% rhsfn = @(x,y,z) uexactfn(x,y,z) .* ( (y.*z).^2+(z.*x).^2+(x.*y).^2 ) .* (cos(x.*y.*z).^2 - sin(x.*y.*z)) - shift*uexactfn(x,y,z); 
...
% rhsfn = @(x,y,z) -3*sin(x).*sin(y).*sin(z) - 3*k^2*sin(k*x).*sin(k*y).*sin(k*z) - shift*uexactfn(x,y,z);
...
uexactfn = @(x,y,z) exp(-x.*(x-1).*y.*(y-1).*z.*(z-1));
dxfn = @(x,y,z) uexactfn(x,y,z).*(1-2*x).*y.*(y-1).*z.*(z-1);
dyfn = @(x,y,z) uexactfn(x,y,z).*(1-2*y).*x.*(x-1).*z.*(z-1);
dzfn = @(x,y,z) uexactfn(x,y,z).*(1-2*z).*x.*(x-1).*y.*(y-1);
rhsfn = @(x,y,z) uexactfn(x,y,z).*( ((1-2*x).*y.*(y-1).*z.*(z-1)).^2 - 2*y.*(y-1).*z.*(z-1) + ...
                                     ((1-2*y).*x.*(x-1).*z.*(z-1)).^2 - 2*x.*(x-1).*z.*(z-1) + ...
                                     ((1-2*z).*x.*(x-1).*y.*(y-1)).^2 - 2*x.*(x-1).*y.*(y-1) ) ... 
                  - shift*uexactfn(x,y,z);
              
nxfn = @(x,y,z) x./sqrt(x.^2+y.^2+z.^2);
nyfn = @(x,y,z) y./sqrt(x.^2+y.^2+z.^2);
nzfn = @(x,y,z) z./sqrt(x.^2+y.^2+z.^2);
h_neumann = @(x,y,z) dxfn(x,y,z).*nxfn(x,y,z) + dyfn(x,y,z).*nyfn(x,y,z) + dzfn(x,y,z).*nzfn(x,y,z);
...
for i = 1:1:n_level
    ...
    F{i}(a_bdyg{i}) = h_neumann(a_xcp{i}(a_bdyg{i}),a_ycp{i}(a_bdyg{i}),a_zcp{i}(a_bdyg{i})).* a_distg{i}(a_bdyg{i})/a_dx{i}^2;
    V{i} = zeros(size(F{i}));
end
...
%     E = interp3_matrix(x1d,y1d,z1d,a_xcp{i}(bdy),a_ycp{i}(bdy),a_zcp{i}(bdy),p,band);
    ...
    L_bdy = (-Ebar/2 + I(bdy,:)/2)/a_dx{i}^2;

    % cubic interp, 3rd order
    % L_bdy = (Edouble/6 - Ebar + E/2 + I(bdy,:)/3) / a_dx{i}^2;
