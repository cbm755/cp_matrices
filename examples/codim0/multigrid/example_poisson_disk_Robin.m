%% test geometric multigrid method to solve -\Delta u  = f in a disk, Robin B.C.s:
% alpha*u + beta*\partial u / \partial n = g 
epsilon = 0;
% q = 6; k = 2;
...
% nyfn = @(x,y) y;
% alpha = 1; beta = 0.5;
% g = @(x,y) alpha*uexactfn(x,y) + beta* ( uxfn(x,y).*nxfn(x,y) + uyfn(x,y).*nyfn(x,y) );
...
R = sqrt(2);
g = @(theta) uexactfn(theta,R) + q*R^(q-1)*gammafn(theta).*sin(k*theta);
...
for i = 1:1:n_level
    ...
    F{i}(a_bdyg{i}) = g(th_cp) .* a_distg{i}(a_bdyg{i}) / a_dx{i}^2;
    %F{i}(a_bdyg{i}) = g(a_xcp{i}(a_bdyg{i}),a_ycp{i}(a_bdyg{i})) .* a_distg{i}(a_bdyg{i}) / a_dx{i}^2;
    V{i} = zeros(size(F{i}));
end
...
for i = 1:1:n_level
    ...
    ng = nnz(bdy);
    diagDist = spdiags(a_distg{i}(a_bdyg{i}),0,ng,ng);
    theta_bdy = cart2pol(a_xcp{i}(bdy),a_ycp{i}(bdy));
    GAMMA = spdiags(gammafn(theta_bdy),0,ng,ng);
    % quadratic interp, 2nd order
    L_bdy = ( diagDist*(Ebar/2+I(bdy,:)/2) + GAMMA*(-Ebar/2 + I(bdy,:)/2) )/dx^2;
