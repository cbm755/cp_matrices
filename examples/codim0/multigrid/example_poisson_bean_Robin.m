%% test geometric multigrid method to solve -\Delta u  = f in a bean-shaped domain, Robin B.C.s
epsilon = 1;
q = 6; k = 2;
...
for i = 1:1:n_level
    ...
    F{i}(a_bdyg{i}) = ( uexactfn(xcp_bdy,ycp_bdy) + ...
        gammafn(xcp_bdy,ycp_bdy).* ( uxfn(xcp_bdy,ycp_bdy).*nxfn(param_bdy) + uyfn(xcp_bdy,ycp_bdy).*nyfn(param_bdy) ) ).* a_distg{i}(a_bdyg{i}) / a_dx{i}^2;
    V{i} = zeros(size(F{i}));
end
...
for i = 1:1:n_level
    ...
    GAMMA = spdiags(gammafn(a_xcp{i}(bdy),a_ycp{i}(bdy)),0,ng,ng);
