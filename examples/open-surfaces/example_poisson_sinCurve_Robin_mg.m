%% Poisson equation on a parametrized sin curve with Robin B.C.s
...
nxfn = @(x) -cos(x)./sqrt(1+cos(x).^2); 
nyfn = @(x) 1./sqrt(1+cos(x).^2);
normal1 = [nxfn(endpt(1)), nyfn(endpt(1))];
normal2 = [nxfn(endpt(2)), nyfn(endpt(2))];
...
rhsfn = @(x) -(cos(x)+k^2*sin(k*x))./(1+cos(x).^2) + (-sin(x)+k*cos(k*x)).*sin(x).*cos(x)./(1+cos(x).^2).^2 - shift*uexactfn(x);
gammafn = @(x) 2*ones(size(x))+sin(x);
h_robin = @(x) uexactfn(x) - gammafn(x).*(-1).^((x-endpt(1))/(endpt(2)-endpt(1))).* (-sin(x)+k*cos(k*x))./sqrt(1+cos(x).^2);

% parameters for multigrid v-cycle scheme
n1 = 5;
n2 = 5;
...
for i = 1:1:n_level
    ...
    nx = zeros(size(xcp_bdy));
    ny = zeros(size(ycp_bdy));
    left = abs(xcp_bdy-endpt(1))<1e-10;
    right = abs(xcp_bdy-endpt(2))<1e-10;
    nx(left) = normal1(1); ny(left) = normal1(2);
    nx(right) = normal2(1); ny(right) = normal2(2);
...
%     yg_bar = 2*yg - a_yg{i}(bdy);
    ...
    GAMMA = spdiags(gammafn(xcp_bdy),0,ng,ng);
    % 2nd order
...
for i = 1:1:n_level
    ...
    F{i}(bdyg) = h_robin(a_xcp{i}(bdyg)) .* a_dist{i} / a_dx{i}^2;
    V{i} = zeros(size(F{i}));
end
