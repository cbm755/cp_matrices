%% Geometric Multigrid Poisson equation on a hemisphere with Robin B.C.
...
gammafn = @(th, phi, r) 2+cos(th);
h_robin = @(th, phi, r) uexactfn(th,phi)-gammafn(th,phi,r).*cos(3*th).*( 3*sin(phi+pi/2).^2.*cos(phi+pi/2).*(9*cos(phi+pi/2).^2-1) - 18*sin(phi+pi/2).^4.*cos(phi+pi/2) )./r;

% uexactfn = @(th, phi) cos(phi+pi/2);
% rhsfn = @(th, phi, r) -2*cos(phi+pi/2)./(r.^2) - shift*uexactfn(th,phi);
% gammafn = @(th, phi, r) 2 + cos(th);
% h_robin = @(th, phi, r) uexactfn(th,phi) + gammafn(th,phi,r).*sin(phi+pi/2)./r;
...
for i = 1:1:n_level
    ...
    xcp_bdy = a_xcp{i}(bdy); ycp_bdy = a_ycp{i}(bdy); zcp_bdy = a_zcp{i}(bdy);
    nx = xcp_bdy/R; ny = ycp_bdy/R; nz = zcp_bdy/R;
    [xg,yg,zg,a_dist{i}] = get_proj_on_conormal_3d(a_xg{i}(bdy),a_yg{i}(bdy),a_zg{i}(bdy),xcp_bdy,ycp_bdy,zcp_bdy,nx,ny,nz);
    xg_bar = 2*xcp_bdy - a_xg{i}(bdy);
    yg_bar = 2*ycp_bdy - a_yg{i}(bdy);
    zg_bar = 2*zcp_bdy - a_zg{i}(bdy);
    ...
    [th, phi, r] = cart2sph(a_xcp{i},a_ycp{i},a_zcp{i});
    GAMMA = spdiags(gammafn(th(bdy), phi(bdy), r(bdy)),0,ng,ng);

    % 2nd order
    M_bdy = D*(I(bdy,:) + Ebar)/2/a_dx{i}^2 + GAMMA*(-Ebar/2 + I(bdy,:)/2)/a_dx{i}^2;
...
for i = 1:1:n_level
    ...
    F{i}(bdyg) = h_robin(th(bdyg), phi(bdyg), r(bdyg)).*a_dist{i}/a_dx{i}^2;
    V{i} = zeros(size(F{i}));
end
