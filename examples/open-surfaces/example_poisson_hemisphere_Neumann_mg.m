%% Geometric Multigrid Poisson equation on a hemisphere with Neumann B.C.
...
h_neumann = @(th, phi, r) -cos(3*th).*( 3*sin(phi+pi/2).^2.*cos(phi+pi/2).*(9*cos(phi+pi/2).^2-1) - 18*sin(phi+pi/2).^4.*cos(phi+pi/2) )./r;

% uexactfn = @(th, phi) cos(phi+pi/2);
% rhsfn = @(th, phi, r) -2*cos(phi+pi/2)./(r.^2) - shift*uexactfn(th,phi);
% h_neumann = @(th, phi, r) sin(phi+pi/2)./r;
...
for i = 1:1:n_level
    ...
    E = interp3_matrix(x1d,y1d,z1d,xcp_bdy,ycp_bdy,zcp_bdy,p,band);
    % quadratic interp, 2nd order
    M_bdy = (-Ebar/2 + I(bdy,:)/2)/a_dx{i}^2;

    % cubic interp, 3rd order
    % E = interp3_matrix(x1d,y1d,z1d,xcp_bdy,ycp_bdy,zcp_bdy,p,band);
    % M_bdy = (Edouble/6 - Ebar + E/2 + I(bdy,:)/3) / a_dx{i}^2;
...
for i = 1:1:n_level
    ...
    F{i}(bdyg) = h_neumann(th(bdyg), phi(bdyg), r(bdyg)).*a_dist{i}/a_dx{i}^2;
    V{i} = zeros(size(F{i}));
end
