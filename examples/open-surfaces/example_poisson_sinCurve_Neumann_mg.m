%% Poisson equation on a parametrized sin curve with Neumann B.C.s
...
h_neumann = @(x) (-sin(x)+k*cos(k*x))./sqrt(1+cos(x).^2);
...
for i = 1:1:n_level
    ...
    F{i}(bdyg) = (-1).^a_bdyg{i}(bdyg) .* h_neumann(a_xcp{i}(bdyg)) .* a_dist{i} / a_dx{i}^2;
    V{i} = zeros(size(F{i}));
end
