...
tf = gpuArray.zeros(size(u_d));
nuut1 = gpuArray.zeros(size(u_d));
tg = gpuArray.zeros(size(u_d));
...
for kt = 1:numtimesteps
  ...
  % TODO: have one call do multiple RHS?
    ...
    %[tf, tg] = feval(gpu_fg, tf, tg, u, v, N);
    tf = f(u_d,v_d);
    tg = g(u_d,v_d);
    %nuut1 = nuu*t1;
    %rhsu_d = tf + nuut1;
