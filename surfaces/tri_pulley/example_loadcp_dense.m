...
[F,V] = plyread('pulley_scaled.ply','tri');

% vertices of the triangles, for plotting
Xp = V(:,1);
Yp = V(:,2);
Zp = V(:,3);
up = sin(12*Zp);  % use interp to get this from 3D grid values
figure(1); clf;
...
dx = 0.05;
p = 3;
grid_data = load( ['pulley_scaled_griddata_p' num2str(p) '_dx' num2str(dx) '.txt' ]);
...
for c=round(rand(1,600)*numband)
