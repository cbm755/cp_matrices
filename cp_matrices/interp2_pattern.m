function [Ei Ej] = interp2_pattern(x, y, xi, yi, p, use_ndgrid)
% Build sparsity pattern for the closest point interpolation matrix
% Just want to see which points are in a certain interpolation stencil 
  ...
  for i = 1:length(xi)
    X = [xi(i), yi(i)];
    [gii,gjj] = buildInterpPattern(X,ptL,ddx,p);
