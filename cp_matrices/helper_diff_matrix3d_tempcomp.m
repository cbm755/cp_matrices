function [L,Lj,Ls] = helper_diff_matrix3d_tempcomp(x, y, z, band1, band2, weights, PTS, ndgrid)
...
%   TODO: maybe pass an opt structure ndgrid, safety etc.
  ...
  if makeListOutput
    L = Li;   % first output is called L
    invbandmap = make_invbandmap(Nx*Ny*Nz, band2);
    Lj = invbandmap(Lj);
  else
